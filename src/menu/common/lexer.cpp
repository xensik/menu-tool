...
#include "xsk/menu/common/lexer.hpp"
#include "xsk/menu/common/error.hpp"

namespace xsk::menu
{

lexer::lexer(std::string const& name, char const* data, usize size) : reader_{ data, size }, loc_{ &name }, buflen_{ 0 }, spacing_{ spacing::null }
{
}
    ...
    while (true)
    {
        ...
        switch (last)
        {
            ...
            case '\n':
                ...
                return token{ token::NEWLINE, spacing_, loc_ };
            case '\\':
                throw parse_error(loc_, "invalid token ('\\')");
            case '/':
                if (curr != '=' && curr != '*' && curr != '/')
                ...
                if (last == '*')
                {
                    ...
                    {
                        if (reader_.ended())
                            throw parse_error(loc_, "unmatched multiline comment start ('/*')");
            ...
            case '*':
                ...
                throw parse_error(loc_, "unmatched multiline comment end ('*/')");
            case '"':
                goto lex_string;
            case '.':
                if (curr == '.')
            ...
            default:
                ...
                throw parse_error(loc_, fmt::format("bad token: '{}'", last));
        }
...
lex_string:
        ...
        {
            if (reader_.ended())
                throw parse_error(loc_, "unmatched string start ('\"')");
            ...
            if (curr == '\n')
                throw parse_error(loc_, "unterminated string literal");
        ...
        if (last == '.' || last != '0' || (last == '0' && (curr != 'o' && curr != 'b' && curr != 'x')))
        {
            ...
            while (true)
            {
                ...
                if (curr == '\'' && (last == '\'' || last == 'f' || last == '.'))
                    throw parse_error(loc_, "invalid number literal");
        ...
        else if (curr == 'o')
        {
            ...
            while (true)
            {
                ...
                if ((curr == '\'' && (last == '\'' || last == 'o')) || (curr == 'o' && last == '\''))
                    throw parse_error(loc_, "invalid octal literal");
        ...
        else if (curr == 'b')
        {
            ...
            while (true)
            {
                ...
                if ((curr == '\'' && (last == '\'' || last == 'b')) || (curr == 'b' && last == '\''))
                    throw parse_error(loc_, "invalid binary literal");
        ...
        else if (curr == 'x')
        {
            ...
            while (true)
            {
                ...
                if ((curr == '\'' && (last == '\'' || last == 'x')) || (curr == 'x' && last == '\''))
                    throw parse_error(loc_, "invalid hexadecimal literal");
