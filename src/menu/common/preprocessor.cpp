...
#include "xsk/menu/common/preprocessor.hpp"
#include "xsk/menu/common/error.hpp"

namespace xsk::menu
{

preprocessor::preprocessor(std::string const& name, char const* data, usize size) : curr_expr_{ 0 }, expand_{ 0 }, skip_{ false }
{
    lexer_.push(lexer{ name, data, size });
    defines_.reserve(4);
...
auto preprocessor::push_header(std::string const& file) -> void
{
    try
    {
        ...
        {
            if (inc == name)
                throw parse_error(location{}, fmt::format("recursive header inclusion {} at {}", name, includes_.back()));
        }
...
auto preprocessor::ban_header(location const& loc) -> void
{
    if (lexer_.size() > 1)
    {
        throw parse_error(loc, "not allowed inside a gsh file");
    }
}
...
auto preprocessor::read_token() -> token
{
    ...
    if (tok.type == token::EOS)
    {
        if (!indents_.empty())
        {
            skip_ = 0;
            // clear indents
            throw parse_error(tok.pos, "missing #endif");
        }
...
auto preprocessor::read_directive(token& tok) -> void
{
    ...
    throw parse_error(next.pos, fmt::format("invalid preprocessing directive '{}'", next.data));
}
...
auto preprocessor::read_directive_elif(token& tok) -> void
{
    if (indents_.empty())
    {
        throw parse_error(tok.pos, "#elif without #if");
    }
    ...
    {
        throw parse_error(tok.pos, "#elif after #else");
    }
...
auto preprocessor::read_directive_elifdef(token& tok) -> void
{
    if (indents_.empty())
    {
        throw parse_error(tok.pos, "#elifdef without #if");
    }
    ...
    if (dir.type == directive::ELSE)
    {
        throw parse_error(tok.pos, "#elifdef after #else");
    }
...
auto preprocessor::read_directive_else(token& tok) -> void
{
    ...
    if (indents_.empty())
    {
        throw parse_error(tok.pos, "#else without #if");
    }
    ...
    if (dir.type == directive::ELSE)
    {
        throw parse_error(tok.pos, "#else after #else");
    }
...
auto preprocessor::read_directive_endif(token& tok) -> void
{
    ...
    if (indents_.empty())
    {
        throw parse_error(tok.pos, "#endif without #if");
    }
...
auto preprocessor::read_directive_define(token&) -> void
{
    ...
    {
        throw parse_error(next.pos, "macro redefinition");
    }
    ...
    switch (next.type)
    {
            ...
            if (next.space == spacing::none)
            {
                ...
                while (true)
                {
                    ...
                    if (next.type == token::RPAREN)
                    {
                        if (last_comma && !params.empty())
                            throw parse_error(next.pos, "misplaced comma in macro param list");
                    ...
                    {
                        if (last_elips)
                            throw parse_error(next.pos, "elipsis must be last in macro param list");
                        else if (!last_comma)
                            throw parse_error(next.pos, "misplaced name in macro param list");
                        else
                        {
                            ...
                            if (it != params.end())
                            {
                                throw parse_error(next.pos, "duplicate macro parameter name");
                            }
                    ...
                    {
                        // TODO: disabled
                        throw parse_error(next.pos, "variadic macros not supported");
                        //
                    ...
                    else if (next.type == token::COMMA)
                    {
                        ...
                        throw parse_error(next.pos, "unexpected token in macro param list");
                }
                ...
                while (next.type != token::NEWLINE)
                {
                    if (next.type == token::NAME)
                    {
                        ...
                        else
                        {
                            // check for #animtree ??
                            if (last_sharp)
                                throw parse_error(next.pos, "'#' is not followed by a macro parameter");
                ...
                if (!exp.empty())
                {
                    if (exp.front().type == token::PASTE)
                        throw parse_error(next.pos, "'##' cannot appear at start of macro expansion");

                    if (exp.back().type == token::PASTE)
                        throw parse_error(next.pos, "'##' cannot appear at end of macro expansion");
        ...
        default:
            ...
            {
                throw parse_error(next.pos, "invalid macro definition");
            }
            break;
    }
}
...
auto preprocessor::read_directive_undef(token& tok) -> void
{
    ...
    if (it != defines_.end())
    {
        if (it->second.type == define::BUILTIN)
            throw parse_error(tok.pos, "can't undefine builtin macro");
...
{
    throw parse_error(tok.pos, "#pragma directive not supported");
}

auto preprocessor::read_directive_warning(token& tok) -> void
{
    throw parse_error(tok.pos, "#warning directive not supported");
}

auto preprocessor::read_directive_error(token& tok) -> void
{
    throw parse_error(tok.pos, "#error directive not supported");
}

auto preprocessor::read_directive_line(token& tok) -> void
{
    throw parse_error(tok.pos, "#line directive not supported");
}
    ...
    else if (def.type == define::FUNCTION)
    {
        ...
        for (auto i = 0u; i < def.exp.size(); i++)
        {
            ...
            else if (def.exp[i].type == token::PASTE)
            {
                ...
                {
                    throw parse_error(def.exp[i].pos, "paste can only be applied to identifiers");
                }
                i++;
            }
            else
...
auto preprocessor::expand_params(token& tok, define& def) -> std::vector<std::vector<token>>
{
    ...
    while (true)
    {
        ...
        {
            throw parse_error(tok.pos, "unterminated function-like macro invocation");
        }
    ...
    {
        throw parse_error(tok.pos, "too few arguments provided to function-like macro invocation");
    }

    if (/*!def.vararg &&*/ args.size() > def.args.size())
    {
        throw parse_error(tok.pos, "too many arguments provided to function-like macro invocation");
    }
...
auto preprocessor::expect(token& tok, token::kind expected, spacing) -> void
{
    ...
    {
        throw parse_error(tok.pos, fmt::format("expected {} found {}", (u8)expected, (u8)tok.type));
    }
}
...
auto preprocessor::evaluate() -> bool
{
    ...
    while (tok.type != token::NEWLINE)
    {
        ...
        else if (tok.type == token::NAME)
        {
            ...
            else
            {
                ...
                {
                    expr_.push_back(token{ token::INT, tok.space, tok.pos, "0" });
                }
            }   
        }
        else
    ...
    {
        throw parse_error(eval_peek().pos, "expected end of line in preprocessor expression");
    }
...
auto preprocessor::eval_consume(token::kind type, std::string_view msg)
{
    ...
    throw parse_error(eval_peek().pos, fmt::format("{}", msg));
}
...
auto preprocessor::eval_expr_factor() -> i32
{
    ...
    while (eval_match(token::STAR) || eval_match(token::DIV) || eval_match(token::MOD))
    {
        ...
        switch (oper.type)
        {
            ...
            case token::DIV:
                if (rval == 0)
                    throw parse_error(oper.pos, "division by 0");
                lval = lval / rval;
                break;
            case token::MOD:
                if (rval == 0)
...
auto preprocessor::eval_expr_primary() -> i32
{
    ...
    if (eval_match(token::DEFINED))
    {
        if (eval_match(token::NAME) || eval_match(token::LPAREN))
        {
            ...
            throw parse_error(eval_peek().pos, "expect identifier after defined(.");
        }

        throw parse_error(eval_peek().pos, "expect identifier or '(' after defined.");
    }

    throw parse_error(eval_peek().pos, "invalid preprocessor expression");
}
